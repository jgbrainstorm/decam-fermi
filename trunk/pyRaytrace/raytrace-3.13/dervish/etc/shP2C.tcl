...
proc param2Chain { args } {
	...
	#I don't split line itself, but need to split a for tcl >= 8.0
	set a $line
	set k [lindex [split $a " "] 0]
	...
	if { $ku == "TYPEDEF" } {
	    ...
	    set done 0
	    regsub {#.*} $line {} line 
            set typetype [string tolower [lindex [split $a " "] 1]]
            set fullline $line
	    while { !$done } {
		...
		if { $structList == "" || [regexp $ku $structList junk]} {
			...
			#echo elem $elem($eltype$j) curval $curval
...
proc chain2Param {args} {
	...
	if { [llength $chnlist ] > 0 } {
		...
		loop i 0 [chainSize $chn] {
		    set cur [chainElementGetByPos $chn $i]
		    set val ${keyPrefix}

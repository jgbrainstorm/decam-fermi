...
#----subtract the median of the row---
...
def master_bias(dir=None,fileHEAD=None,medianName=None):
    ...
    for i in range(1,NChannel):     # i runs for channel
        for j in range(0,Nfile):    # j rDECAMWS01 cannot loginuns for file
...
#-------------linearity----------------------
#This is old version of the code as of 7/28/11. The left and right for north and south CCDs are not defined in a consistent way. i.e., for N CCD, left corresponding to the right of the S CCD. Left is defined as the right side of the charge flow direction. See notebook for details.
#--------------------------------------------

def linearity_old(NameFits,NameBias,Channel,shift=None,left=None):
    ...
    if detector[-1]=='N':   # this is the old one and now rotated back to this on 7/25/11
...
#-------new version----on 7/28/11


def linearity(NameFits,NameBias,Channel,shift=None,left=None):
    #This is the new one on 7/28/11 to correct the left/right
    detector = pf.open(NameBias)[Channel].header['DETPOS']
    #before the 180 rotation on 6/2011
    
    if detector[0]=='N':
        if left == None or left == 1:
            colmin=300
            colmax=800
            rowmin=3946  #new region
            rowmax=4046
        else:
            colmin=1360
            colmax=1860
    ...
    #After the 180 rotation on 6/2011
    """
    if detector[-1]=='N':
        if left == None or left == 1:
            colmin=1360
